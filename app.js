...
let state = {
  ...
  features: { live:true, keepers:true, biases:false, stack:false, compare:false, scarcity:true, advanced:false },
...
async function loadConsensus() {
  try {
    ...
    // If no projections, hide compare by default
    if(!state.dataFlags.hasProj){ state.features.compare = false; el("toggleCompare").checked = false; }
    // If no projections or ADP, reduce the recs detail automatically
    render();
  } catch (e) {
...
function updateFeatureVisibility(){
  ...
  // filter is always useful; hidden only until data loaded
  show("filterWrap", state.players.length>0);
  show("advScoringWrap", state.features.advanced);
}
...
function aiPick(teamIndex){
  ...
  // Bias handling
  const bias = state.features.biases ? (state.biasMap[teamIndex+1] || "BAL") : "BAL"; // "BAL" = balanced
  const weighted = list.slice(0,24).map(p=> ({...p, biasScore: p.score + biasBoost(p, bias)}));
  weighted.sort((a,b)=>b.biasScore-a.biasScore);
  // Weighted randomness among top 7
...
function computeRecommendations(teamIndex){
  ...
  const scored = candidates.map(p=>{
    ...
    const score = (state.dataFlags.hasProj ? vor*needW : 0) + tierBoost + valueBoost;
...
function renderRecs(){
  ...
  list.forEach((p)=>{
    ...
    // Only show VOR line if projections exist
    ...
    d.querySelector("button").onclick=()=>{
      if(state.features.live && overallToTeam(state.currentOverall)!==state.myTeamIndex){
        alert("Live Mode is on. Use Manual Pick for other teams.");
        return;
      }
