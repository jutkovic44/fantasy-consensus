/* Fantasy Football War Room — Manual vs. Regular Draft Mode + simplified controls */
...
function hasPrimaryStackForMyTeam(candidate) {
  ...
  for (const idx of rosterIdxs) {
    ...
    if (
      (candidate.pos === "QB" &&
        (pl.pos === "WR" || pl.pos === "TE")) ||
      ((candidate.pos === "WR" || candidate.pos === "TE") &&
        pl.pos === "QB")
    ) {
      return true;
    }
...
function stackBonusForTeam(teamIndex, candidate) {
  ...
  for (const pl of roster) {
    ...
    if (
      (pl.pos === "QB" &&
        (candidate.pos === "WR" || pl.pos === "TE")) ||
      (candidate.pos === "QB" &&
        (pl.pos === "WR" || pl.pos === "TE"))
    ) {
      bonus += 6;
    } else if (
      pl.pos === candidate.pos &&
      (pl.pos === "WR" || pl.pos === "TE")
    ) {
      bonus += 2;
    }
...
function byeOverlapPenalty(teamIndex, candidate) {
  ...
  const fillingLastStarter =
    list.length + 1 >= (startersTarget[candidate.pos] || 0);
...
function computeScarcityBoost(p) {
  ...
  const remain = state.available
    .map(i => state.players[i])
    .filter(x => x.pos === p.pos).length;
  if (total === 0) return 0;
...
function lateRoundUpsideBonus(p) {
  ...
  const discount =
    state.dataFlags.hasADP && p.adp
      ? Math.max(0, p.adp - state.currentOverall)
      : 0;
  if (discount <= 0) return 0;
  const tier = p.tier || 6;
  const tierLean = Math.max(0, tier - 2);
  const deep =
    pct >= WEIGHTS.deepRoundStartPct ? 1.25 : 1.0;
  return (
    Math.log10(1 + discount) * (0.75 + 0.25 * tierLean) * deep
  );
}
...
async function loadConsensus() {
  ...
  try {
    ...
    if (!resp.ok)
      throw new Error(`HTTP ${resp.status} — consensus.json not found`);
    const data = await resp.json();
    if (!Array.isArray(data.players))
      throw new Error("consensus.json missing 'players' array");
...
function ingestPlayers(raw) {
  ...
  state.dataFlags.hasProj = state.players.some(
    p => (p.proj_ppr || 0) > 0
  );
  state.dataFlags.hasADP = state.players.some(
    p => p.adp !== null && p.adp !== undefined
  );
  buildPosRankCache();
}
  ...
  if (btn) {
    btn.addEventListener("click", () => {
      if (!state.players.length) {
        alert(
          "No players loaded yet. Upload a CSV first or ensure consensus.json has players."
        );
        return;
      }
...
function parseCsvFile(file) {
  ...
  reader.onload = () => {
    ...
    const players = rows
      .map(r => mapFantasyProsRow(headers, r))
      .filter(Boolean);
    if (!players.length) {
      alert(
        "Could not parse any rows from CSV. Check the file format."
      );
      return;
    }
    el("dataSourceLabel").textContent = `CSV: ${file.name}`;
    el("lastUpdated").textContent = `Loaded from CSV: ${
      players.length
    } rows`;
  ...
  const get = aliases => {
    for (const a of aliases) {
      const idx = headers.findIndex(
        h => h.toLowerCase() === a.toLowerCase()
      );
      if (idx >= 0) return row[idx];
    }
  };
  const player = get(["Player", "PLAYER", "Name"]);
...
function init() {
  ...
  ["teams", "rounds", "pickPos", "scoring",
   ...
   "flexSlots", "kSlots", "defSlots", "benchSlots"
  ].forEach(id =>
    el(id)?.addEventListener("input", syncSettings)
  );
  ...
  el("tabOverall")?.addEventListener("click", () => {
    ...
    localStorage.setItem("boardView", "overall");
    updateBoardTabs();
    renderBoard();
  });
  el("tabByRound")?.addEventListener("click", () => {
    state.boardView = "round";
    localStorage.setItem("boardView", "round");
    updateBoardTabs();
    renderBoard();
  });
...
// ---------- draft controls rendering ----------
function renderDraftControls() {
  const ctrl = el("draftControls");
  if (state.settings.manualMode) {
    // show manual controls
    ctrl.querySelector("#nextPick").textContent = "Pick for Team " + (state.settings.pickPos);
  } else {
    // show regular controls
    ctrl.querySelector("#nextPick").textContent = "Next Pick";
  }
}
...
function startMock() {
  ...
  state.rosterSlots = new Array(T).fill(0).map(() => ({
...
function undoPick() {
  ...
  if (!state.available.includes(playerIdx))
  ...
  if (pos in state.rosterSlots[team])
    state.rosterSlots[team][pos] = Math.max(
      0,
      state.rosterSlots[team][pos] - 1
    );
...
function exportBoard() {
  ...
  for (const p of [...state.draftPicks].sort(
...
function replacementLevels() {
  ...
  const s = state.settings,
    T = s.teams,
    flexShare = s.flex;
  const counts = { QB: s.qb, RB: s.rb, WR: s.wr, TE: s.te, K: s.k, DEF: s.def },
    idxAt = {};
  for (const pos of ["QB", "RB", "WR", "TE", "K", "DEF"]) {
    let N = T * counts[pos];
    if (pos === "RB" || pos === "WR") {
      N += Math.round(T * (0.5 * flexShare));
    }
...
function applyFilters(list) {
  ...
  if (pos) out = out.filter(p => (p.pos || "").toUpperCase() === pos);
...
function computeRecommendations(teamIndex) {
  ...
  const scored = candidates.map(p => {
    ...
    const resulting =
      p.bye != null ? (countsNow.get(p.bye) || 0) + 1 : 0;
...
function playerCardHTML(p) {
  ...
  const adpBit = state.dataFlags.hasADP
    ? ` • ADP ${p.adp || "-"}`
    : "";
  const projBit = state.dataFlags.hasProj
    ? ` • Proj ${Number(p.baseProj ?? p.proj_ppr || 0).toFixed(
        1
      )}` +
      (p.rep != null
        ? ` (rep ${Number(p.rep).toFixed(1)})`
        : "")
    : "";
  const stackBadge =
    p.hasMyStack || hasPrimaryStackForMyTeam(p)
  ...
  return `<div class="flex">
      <div class="flex" style="gap:10px;">
        ${
          logo
            ? `<img src="${logo}" alt="${p.team || ""}" class="team-logo">`
            : ""
        }
        <div>
          <div class="name">${p.player} ${stackBadge} ${
    upgradeBadge
  }
            <span class="badge tier t${t}">T${t}</span>
            <span class="badge pos ${p.pos}">${p.pos}${
    pr ? posRankLabel(pr) : ""
  }</span>
            <span class="badge">${ecrText}</span>
          </div>
          <div class="small">${p.team || ""} • Bye ${
    p.bye || "-"
  } ${byeDot}${adpBit}${projBit}</div>
        </div>
      </div>
...
function renderBoard() {
  ...
  if (!root) return;
  root.innerHTML = "";
  const picks = [...state.draftPicks].sort(
    (a, b) => a.overall - b.overall
  );
  if (state.boardView === "overall") {
    ...
    Array.from(byRound.keys())
      ...
      .forEach(r => {
        ...
        root.appendChild(h);
        byRound.get(r).forEach(p =>
          root.appendChild(boardPickElem(p))
        );
      });
  }
}
...
function boardPickElem(p) {
  ...
  div.className = "pick";
  div.innerHTML = `<div class="flex"><span class="badge">#${p.overall} R${p.round}.${p.pickInRound}</span><span class="small">Team ${p.team +
    1}</span></div>
                   <div class="flex" style="justify-content:flex-start; gap:8px;">
                     ${
                       logo
                         ? `<img src="${logo}" alt="${pl.team ||
                             ""}" class="team-logo">`
                         : ""
                     }
                     <div class="name">${pl.player}</div>
                   </div>
                   <div class="small"><span class="badge pos ${pl.pos}">${pl.pos}${
    pr ? posRankLabel(pr) : ""
  }</span> • ${pl.team || ""} • Bye ${pl.bye || "-"}</div>`;
  return div;
}
...
function renderMidPanel() {
  ...
  if (state.midTab === "recs") {
    ...
    const countsNow = byeOverlapCounts(
      startersAllForTeam(state.myTeamIndex)
    );
...
function renderMyRoster() {
  ...
  mine.forEach(p => {
    if (slotsTarget[p.pos] && starters[p.pos].length < slotsTarget[p.pos]) {
      ...
      (p.pos === "RB" || p.pos === "WR") &&
      starters.FLEX.length < slotsTarget.FLEX
    ) {
  ...
  bench.sort((a, b) => {
    const pa = positionOrder(a.pos),
      pb = positionOrder(b.pos);
    if (pa !== pb) return pa - pb;
    return benchValue(b) - benchValue(a);
  });

  const startersAll = startersAllForTeam(state.myTeamIndex);
  const counts = byeOverlapCounts(startersAll);

  function section(label, list, target, benchMode = false) {
    let headerBadges = "";
    if (!benchMode) {
...
function boardPickElem(p) {
  ...
  }<div class="name">${pl.player}</div></div>
  <div class="small"><span class="badge pos ${pl.pos}">${pl.pos}${
    pr ? posRankLabel(pr) : ""
  }</span> • ${pl.team || ""} • Bye ${pl.bye || "-"}</div>`;
  return div;
